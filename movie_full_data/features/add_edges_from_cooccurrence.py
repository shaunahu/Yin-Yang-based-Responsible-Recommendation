#!/usr/bin/env python3
# add_edges_from_cooccurrence_movies.py
# Add co-occurrence edges to the MOVIES graph from impression_cooccurrence_movies.csv
# Dec 11, 2025

import os
import argparse
import pandas as pd
import networkx as nx  # only needed if you plan to inspect types, not strictly required
...
def main(min_count: int, overwrite: bool):
    ...
    GRAPH_DIR = os.path.abspath(os.path.join(HERE, "..", "moviesGraph"))
    EDGE_DIR  = os.path.abspath(os.path.join(HERE, "..", "edge_data"))

    GRAPH_IN   = os.path.join(GRAPH_DIR, "graph.pkl")
    COOC_CSV   = os.path.join(EDGE_DIR, "impression_cooccurrence_movies.csv")
    ...
    df = pd.read_csv(
        COOC_CSV,
        dtype={"movieid_a": str, "movieid_b": str, "cooccurrence": int}
    )
    required = {"movieid_a", "movieid_b", "cooccurrence"}
    if not required.issubset(df.columns):
        raise ValueError(
            f"{COOC_CSV} must have columns: {sorted(required)}; found {list(df.columns)}"
        )
    ...
    print("[INFO] Adding edges with 'cooccurrence' attribute (movies dataset) ...")
    for _, row in tqdm(df.iterrows(), total=len(df), desc="Adding edges"):
        u, v, c = row["movieid_a"], row["movieid_b"], int(row["cooccurrence"])
    ...
    meta = {
        ...
        "min_count": min_count,
        "note": (
            "Each edge has attribute 'cooccurrence' = number of times the two movies "
            "co-occurred in impression lists (movies dataset)."
        ),
    }
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(
        description="Add edges to MOVIES graph from impression_cooccurrence_movies.csv"
    )
    ap.add_argument(
        "--min_count",
        type=int,
    ...
    ap.add_argument(
        "--overwrite",
        action="store_true",
        help="Overwrite moviesGraph/graph.pkl instead of writing graph_with_edges.pkl",
    )
    args = ap.parse_args()
