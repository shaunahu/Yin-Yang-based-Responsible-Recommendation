...
# balanced clustering: allow ±TOLERANCE deviation from perfect equal size
CLUSTER_SIZE_TOLERANCE = 0.2
...
def run_kmeans(embeddings: np.ndarray):
    ...
    sizes_before = [int(np.sum(labels == c)) for c in range(NUM_CLUSTERS)]
    print(f"  KMeans sizes (before balance): {sizes_before}")

    # ---- balanced post-processing ----
    labels, centroids = balance_clusters(embeddings, labels, centroids)

    sizes_after = [int(np.sum(labels == c)) for c in range(NUM_CLUSTERS)]
    print(f"  Balanced sizes (after):        {sizes_after}")

    inertia = float(np.sum(
        np.linalg.norm(embeddings - centroids[labels], axis=1) ** 2
    ))
...
def balance_clusters(
    ...
    """Move nodes from oversized clusters to undersized ones until balanced.

    Each iteration picks the largest cluster, finds the node in it that is
    closest to the centroid of the smallest cluster, and reassigns it.
    Centroids are updated incrementally after each move.
    Stops when all clusters are within the tolerance or max_rounds is reached.
    """
    n = len(labels)
    labels = labels.copy()
    centroids = centroids.copy()
    ideal = n / NUM_CLUSTERS
    lo = max(1, int(ideal * (1 - CLUSTER_SIZE_TOLERANCE)))
    hi = int(math.ceil(ideal * (1 + CLUSTER_SIZE_TOLERANCE)))

    sizes = np.array([int(np.sum(labels == c)) for c in range(NUM_CLUSTERS)])

    for _ in range(max_rounds):
        if sizes.max() <= hi and sizes.min() >= lo:
            break
        ...
        # indices of nodes in the biggest cluster
        big_idx = np.where(labels == big)[0]

        # find the node in big cluster closest to the small cluster centroid
        dists_to_small = np.linalg.norm(
            embeddings[big_idx] - centroids[small], axis=1
        )
        best_local = int(np.argmin(dists_to_small))
        move_idx = big_idx[best_local]

        # update centroids incrementally
        old_big_size = sizes[big]
        old_small_size = sizes[small]

        if old_big_size > 1:
            centroids[big] = (
                centroids[big] * old_big_size - embeddings[move_idx]
            ) / (old_big_size - 1)

        centroids[small] = (
            centroids[small] * old_small_size + embeddings[move_idx]
        ) / (old_small_size + 1)
...
def main():
    ...
    manifest = {
        "version": "v4",
        "method": "largest_connected_component + robust_minmax_edge_fusion + threshold + symmetric_topk + weighted_node2vec + kmeans_balanced",
        ...
        "clustering": {
            "method": "kmeans_balanced",
            "size_tolerance": CLUSTER_SIZE_TOLERANCE,
            **KMEANS_PARAMS,
        },
        "quality": {
